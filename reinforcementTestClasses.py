...
class QLearningExplorationTest(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(QLearningExplorationTest, self).__init__(question, testDict)
        self.discount = float(testDict['discount'])
        self.grid = gridworld.Gridworld(parseGrid(testDict['grid']))
        ...
        self.env = gridworld.GridworldEnvironment(self.grid)
        self.learningRate = float(testDict['learningRate'])
        self.k = float(testDict['k'])
        self.opts = {'actionFn': self.env.getPossibleActions, 'gamma': self.discount, 'alpha': self.learningRate}
        self.testOutFile = testDict['test_out_file']

        if 'experiences' in testDict:
            experiences_list = eval(testDict['experiences'])
            self.experiences = Experiences()
            self.experiences.set_experiences(experiences_list)
            numExperiences = len(experiences_list)
        else:
            numExperiences = int(testDict['numExperiences'])
            if sys.platform == 'win32':
                _, question_name, test_name = testDict['test_out_file'].split('\\')
        ...
        maxPreExperiences = 10
        self.numsExperiencesForDisplay = list(range(min(numExperiences, maxPreExperiences)+1))
        if maxPreExperiences < numExperiences:
            self.numsExperiencesForDisplay.append(numExperiences)
    ...
    def executeNExperiences(self, grades, moduleDict, solutionDict, n, checkValuesAndPolicy):
        testPass = True
        # Reset experience list before running again
        self.experiences.reset()
        valuesPretty, qValuesPretty, actions, policyPretty, lastExperience, agent = self.runAgent(moduleDict, n)
        stdOutString = ''
        # fileOutString = "==================== Iteration %d ====================\n" % n
        fileOutString = ''
        if lastExperience is not None:
            ...
            pass
        for action in actions:
            qValuesKey = 'q_values_after_%d_updates_action_%s' % (n, action)
            qValues = qValuesPretty[action]
        ...
        if checkValuesAndPolicy:
            if not self.comparePrettyValues(valuesPretty, solutionDict['values']):
                ...
                stdOutString += outString
                fileOutString += outString
            if not self.comparePrettyValuesSet(policyPretty, solutionDict['exploration_policy']):
                testPass = False
                outString = "Exploration policy (from getAction(state)) is NOT correct."
                outString += "   Student solution:\n\t%s" % self.prettyValueSolutionString('exploration_policy', policyPretty)
                outString += "   Correct solution:\n\t%s" % self.prettyValueSolutionString('exploration_policy', solutionDict['exploration_policy'])
                stdOutString += outString
                fileOutString += outString
        return testPass, stdOutString, fileOutString

    def getSolutionPolicy(self, agent):
        policy = {}
        states = [state for state in self.grid.getStates() if len(self.grid.getPossibleActions(state)) > 0]
        states.sort()
        for state in states:
            policy[state] = ','.join(agent.getActions(state))
        return policy
    ...
    def writeSolution(self, moduleDict, filePath):
        with open(filePath, 'w') as handle:
            ...
            for n in self.numsExperiencesForDisplay:
                # Reset experience list before running again
                self.experiences.reset()
                valuesPretty, qValuesPretty, actions, policyPretty, _, agent = self.runAgent(moduleDict, n)
                for action in actions:
                    handle.write(self.prettyValueSolutionString('q_values_after_%d_updates_action_%s' % (n, action), qValuesPretty[action]))
            solutionPolicyPretty = self.prettyPolicy(self.getSolutionPolicy(agent))
            handle.write(self.prettyValueSolutionString('values', valuesPretty))
            #handle.write(self.prettyValueSolutionString('exploration_policy', policyPretty))
            handle.write(self.prettyValueSolutionString('exploration_policy', solutionPolicyPretty))
        return True

    def runAgent(self, moduleDict, numExperiences):
        agent = moduleDict['qlearningAgents'].QLearningAgentCountExploration(k=self.k, **self.opts)
        states = [state for state in self.grid.getStates() if len(self.grid.getPossibleActions(state)) > 0]
        states.sort()
        lastExperience = None
        for i in range(numExperiences):
        ...
        for state in states:
            values[state] = agent.computeValueFromQValues(state)
            # Collect exploration policy rather than policy based only on q-values
            #policy[state] = agent.computeActionFromQValues(state)
        ...
        for action in actions:
            qValuesPretty[action] = self.prettyValues(qValues[action])
        return (valuesPretty, qValuesPretty, actions, policyPretty, lastExperience, agent)
    ...
    def comparePrettyValuesSet(self, aPretty, bPretty, tolerance=0.01):
        aList = self.parsePrettyValues(aPretty)
        bList = self.parsePrettyValues(bPretty)
        if len(aList) != len(bList):
            return False
        for a, b in zip(aList, bList):
            try:
                ...
                if a.strip() not in b.strip().split(','):
                    return False
        return True
