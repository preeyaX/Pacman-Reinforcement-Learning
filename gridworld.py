...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def getPossibleActions(self, state):
        ...
        if state == self.grid.terminalState:
            return ()
        x,y = state
        if x < self.grid.width and y < self.grid.height and type(self.grid[x][y]) == int:
            return ('exit',)
...
if __name__ == '__main__':
    ...
    if opts.agent == 'value':
        ...
        qLearnOpts = {'gamma': opts.discount,
                      ...
                      'epsilon': opts.epsilon,
                      'actionFn': actionFn}
        a = qlearningAgents.QLearningAgentCountExploration(**qLearnOpts)
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if (opts.agent == 'q' or opts.agent == 'qe'): displayCallback = lambda state: display.displayQValues(a, state, "CURRENT Q-VALUES")
    ...
    if (opts.agent == 'q' or opts.agent == 'qe') and not opts.manual:
        try:
